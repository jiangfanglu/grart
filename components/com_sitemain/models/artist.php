...
class SitemainModelArtist extends JModelItem
{
    ...
    public function getReviews($user_id){
        ...
        $query = "SELECT ap.product_id, r.text, r.date_added, customer_id, ap.artwork_id, ai.title 
                ...
                where ai.user_id = ".$user_id." and r.status = 1 limit 5";
        $db->setQuery($query);
    ...
    public function getArtworks($status = null, $artist_id=null){
        ...
        if($status == 1){
            ...
            $query->where('ATI.user_id='.(string)$artist_id);
        }
    ...
    public function addArtistFollower($data){
        $artist_id = $data['artist_id'];
        $follower_id = $data['follower_id'];
        
        $db = JFactory::getDbo();
        $query = $db->getQuery(true);
        $query = 'insert into #__artist_follower (artist_user_id, follower_user_id) 
            values ('.(string)$artist_id.','.(string)$follower_id.')';
        $db->setQuery($query);
        $db->query();
    }
    ...
    public function getFollowers($a_id){
        ...
        $query = "select u.name as uname, af.* from #__artist_follower as af
                    ...
                    where af.artist_user_id = ".(string)$a_id." limit 5";
        $db->setQuery($query);
