 ...
 * HelloWorld Model
 */
class SitemainModelArtwork extends JModelItem
{
    ...
    public function setPrimaryImage($artwork_id, $artwork_image_id){
        $db = JFactory::getDbo();
        $query = $db ->getQuery(true);
        $query = "select id from #__artwork_images where artwork_id=".$artwork_id;
        $db ->setQuery($query);
        $images = $db->loadObjectList();
        
        foreach($images as $img){
            $query = $db ->getQuery(true);
            if((int)$img->id == (int)$artwork_image_id){
                $query = "update #__artwork_images set hero=1 where artwork_id=".$artwork_id." and id=".(string)$img->id;
            }else{
                $query = "update #__artwork_images set hero=0 where artwork_id=".$artwork_id." and id=".(string)$img->id;
            }
            $db ->setQuery($query);
            try{
                $db->query();
