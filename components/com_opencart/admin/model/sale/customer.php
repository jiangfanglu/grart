...
class ModelSaleCustomer extends Model {
		...
		if($username_exists==""){
			...
			if($new_joomla_version=="Yes"){ //if joomla version is greater than>=1.6
				...
				if(!$joomla_acl_aro_id){
					$joomla_db->query("INSERT INTO " . $JConfig->dbprefix . "core_acl_aro SET section_value = 'users',value = '" . $joomla_user_id . "',order_value = '0', name = '".$joomla_db->escape($data['firstname']) . " " . $joomla_db->escape($data['lastname'])."', hidden = '0'");					$joomla_acl_aro_id=$joomla_db->getLastId();				}				$joomla_db->query("INSERT INTO " . $JConfig->dbprefix . "core_acl_groups_aro_map SET group_id = '".$user_group_id."',section_value = '',aro_id = '".$joomla_acl_aro_id."'");
	...
	public function editCustomer($customer_id, $data) {
		...
		global $joomla_db;
		$block=$data['status']=="1"?"0":"1";
		$result = $this->db->query("SELECT email FROM ".DB_PREFIX . "customer WHERE customer_id = '" . (int)$customer_id . "'");
		if($result->num_rows){
			$customer_email=$result->row["email"];	
		}
		if(	$customer_email!=""){		
			$JConfig = new JConfig();
			$joomla_db->query("UPDATE " . $JConfig->dbprefix . "users SET name = '" . $joomla_db->escape($data['firstname']) . " " . $joomla_db->escape($data['lastname']) . "',email = '" . $joomla_db->escape($data['email']) . "',block='".$block."'		 WHERE email='".$joomla_db->escape($customer_email)."'");		 $joomla_db->query("UPDATE " . $JConfig->dbprefix . "users SET username = '" . $joomla_db->escape($data['email']) . "' WHERE username='".$joomla_db->escape($customer_email)."'");
		}
      	...
      	if ($data['password']) {
			...
			$joomla_db->query("UPDATE " . $JConfig->dbprefix . "users SET password = '" . $encrypted_password . "' WHERE email='".$joomla_db->escape($customer_email)."'");
	...
	public function deleteCustomer($customer_id) {
		...
		if(	$customer_email!=""){		
			$JConfig = new JConfig();
			$joomla_db->query("DELETE FROM " . $JConfig->dbprefix . "users where email='".$customer_email."'");
		}
		//End delete from joomla users table
	...
	public function approve($customer_id) {
		...
		if ($customer_info) {
			...
			if ($store_info) {
				...
				$store_url = $store_info['url'] . 'index.php?option=com_opencart&route=account/login';
			} else {
				$store_name = $this->config->get('config_name');
				$store_url = HTTP_CATALOG . 'index.php?option=com_opencart&route=account/login';
			}
		...
		if(	$customer_email!=""){		
			$JConfig = new JConfig();
			$joomla_db->query("UPDATE " . $JConfig->dbprefix . "users SET activation='', block='".$block."'
