...
class ModelCatalogProduct extends Model {
	...
	public function deleteProduct($product_id) {
        ...
        public function changeArtworkStatus($product_id){
               $db = JFactory::getDbo();
                $query = $db->getQuery(true);
                $query = "update #__artwork_publish set status=2 where product_id=".$product_id;
                $db ->setQuery($query);
                try{
                    $db->query();
                }catch(Exception $e){
                ...
                $query = $db->getQuery(true);
                $query = "select artwork_id from #__artwork_publish where product_id=".$product_id;
                $db ->setQuery($query);
                $artwork = $db->loadObject();
                
                $query = $db->getQuery(true);
                $query = "update #__artwork_info set status=2 where id=".$artwork->artwork_id;
                $db ->setQuery($query);
                try{
                    $db->query();
                }catch(Exception $e){
