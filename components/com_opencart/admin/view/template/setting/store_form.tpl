        ...
        <div id="tab-store">
          <table class="form">
            ...
            <tr>
              ...
              <td><select name="config_template" onchange="$('#template').load('index.php?option=com_opencart&route=setting/store/template&token=<?php echo $token; ?>&template=' + encodeURIComponent(this.value));">
...
function image_upload(field, thumb) {
	...
	$('#content').prepend('<div id="dialog" style="padding: 3px 0px 0px 0px;"><iframe src="index.php?option=com_opencart&route=common/filemanager&token=<?php echo $token; ?>&field=' + encodeURIComponent(field) + '" style="padding:0; margin: 0; display: block; width: 100%; height: 100%;" frameborder="no" scrolling="auto"></iframe></div>');
	...
	$('#dialog').dialog({
		...
		close: function (event, ui) {
			if ($('#' + field).attr('value')) {
				$.ajax({
					url: 'index.php?option=com_opencart&route=common/filemanager/image&token=<?php echo $token; ?>&image=' + encodeURIComponent($('#' + field).val()),
					dataType: 'text',
