...
class ControllerUserUser extends Controller {  
	...
	private function getForm() {
		...
		// start joomla user groups
		if(defined("DONT_SHOW_ADMIN_LOGIN") && DONT_SHOW_ADMIN_LOGIN=="1" && isset($_REQUEST["route"]) && $_REQUEST["route"]=="user/user/insert"){		
			$j_config=new JConfig();
			if(version_compare(JVERSION, '1.6.0', '<' ) != 1){
				$tabl_name=$j_config->dbprefix."usergroups";
				$field_name="title";
			} else{
				$tabl_name=$j_config->dbprefix."core_acl_aro_groups";
				$field_name="value";
			}
			$query=$this->db->query("select id as joomla_user_group_id,".$field_name." as name from ".$tabl_name);
			$joomla_user_groups=$query->rows;
			$this->data['joomla_user_groups']=$joomla_user_groups;	
		}	
		// end joomla user groups
