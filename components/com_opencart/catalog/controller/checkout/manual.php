...
class ControllerCheckoutManual extends Controller {
	public function index() {
		...
		$user_token_query = $this->db->query("SELECT code FROM " . DB_PREFIX . "user WHERE code = '" .$this->request->get['token']. "' AND status = '1'");
		if($user_token_query->num_rows){
			$allow_user="yes";
		}else{
			$allow_user="no";
		}
		
		if ($allow_user=="yes" || ($this->user->isLogged() && $this->user->hasPermission('modify', 'sale/order'))) {
			// Reset everything
			$this->cart->clear();
