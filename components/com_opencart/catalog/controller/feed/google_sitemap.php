...
class ControllerFeedGoogleSitemap extends Controller {
   public function index() {
	  if ($this->config->get('google_sitemap_status')) {
		 ...
		 foreach ($manufacturers as $manufacturer) {
			...
			foreach ($products as $product) {
			   $output .= '<url>';
			   $output .= '<loc>' . str_replace("&","&amp;",str_replace("&amp;","&",$this->url->link('product/product', 'manufacturer_id=' . $manufacturer['manufacturer_id'] . '&product_id=' . $product['product_id']))) . '</loc>';
			   $output .= '<changefreq>weekly</changefreq>';
			   $output .= '<priority>1.0</priority>';
			   $output .= '</url>';   
			}         
   ...
   protected function getCategories($parent_id, $current_path = '') {
	  ...
	  foreach ($results as $result) {
		 ...
		 foreach ($products as $product) {
			$output .= '<url>';
			$output .= '<loc>' . str_replace("&","&amp;",str_replace("&amp;","&",$this->url->link('product/product', 'path=' . $new_path . '&product_id=' . $product['product_id']))) . '</loc>';
			$output .= '<changefreq>weekly</changefreq>';
			$output .= '<priority>1.0</priority>';
			$output .= '</url>';   
		 }   
