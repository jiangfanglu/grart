...
class ModelCatalogProduct extends Model {
	public function updateViewed($product_id) {
        ...
        public function getArtistAndOtherProducts($product_id) {
                $db = JFactory::getDbo();
                $query = $db->getQuery(true);
                $query = "SELECT user_id, u.name FROM #__artwork_publish ap
                            inner join #__artwork_info ai on ap.artwork_id = ai.id
                            right join #__users u on u.id = ai.user_id
                ...
                if($artist!=null){
                    $query = $db->getQuery(true);
                    $query = "SELECT ap.product_id, title, image FROM #__artwork_publish ap
                                inner join #__artwork_info ai on ap.artwork_id = ai.id 
                                left join oc_product op on op.product_id = ap.product_id
                                where user_id = ".$artist->user_id." and ap.status = 1 and ai.status=1";
                    $db ->setQuery($query);
                    $other_products = $db ->loadObjectList();
        ...
        public function getSold($product_id){
                ...
                $query = "select sum(quantity) as total
                        ...
                        where op.product_id = ".$product_id." and oo.order_status_id = 5";
                $db ->setQuery($query);
                $totalsold = $db ->loadObject();
                return $totalsold;
        }
        ...
        public function getCategory($product_id){
                ...
                $query = "select cd.category_id, cd.name
                        ...
                        right join oc_category_description cd 
