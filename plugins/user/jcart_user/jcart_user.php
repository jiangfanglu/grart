...
function plgjcart_user_login($user, $options = array())
{
	...
	$_SESSION['jcart_password']=md5($user['password']);
	return true;
}
...
function plgjcart_user_before_store($old_user, $isnew, $new_user)
{
	if(!$isnew){
		require_once(JPATH_SITE."/components/com_opencart/index_mod.php");
		global 	$db_jcart;	
		$db_jcart = new DB(DB_DRIVER, DB_HOSTNAME, DB_USERNAME, DB_PASSWORD, DB_DATABASE);
...
function plgjcart_user_store($user, $isnew, $succes, $msg)
{
	...
	$jcart_password   = md5($user['password_clear']);
	...
	if ($customer_query->num_rows) {
		...
		$db_jcart->query("UPDATE " . DB_PREFIX . "customer SET password = '" . $db_jcart->escape($jcart_password) . "',firstname = '" . $db_jcart->escape($fname) . "', lastname = '" . $db_jcart->escape($lname) . "' ".$status_str.",email='".$db_jcart->escape($data['email'])."' WHERE email = '" . $db_jcart->escape($data['email']) . "'");	
